...
const translationRange = 0.85;
let direction = 1;

// for rotation of the windmill and Moon
let rotationAngle = 0.0;

let rotationAngle_moon = 0.0;

const rotationSpeed = 0.06;

const rotationSpeed_Moon = 0.02;
...
function drawRoad() {
    ...
    mat4.identity(mMatrix);
    pushMatrix(matrixStack, mMatrix);
    color = [0.30, 0.57, 0, 0.9];
    mMatrix = mat4.translate(mMatrix, [0.53, -0.82, 0]);
    mMatrix = mat4.rotate(mMatrix, 7.13, [0, 0, 1]);
    mMatrix = mat4.scale(mMatrix, [1.8, 2, 1.0]);
    drawTriangle(color, mMatrix);
    mMatrix = popMatrix(matrixStack);
}

function drawTrees(move = false, t_x = 0, t_y = 0, s_x = 0, s_y = 0) {
    // Uniform scaling factors for the tree
    const treeScaleX = 1.15; // Change this value for uniform X scaling
    const treeScaleY = 1.15; // Change this value for uniform Y scaling
    ...
    if (move) {
        ...
        // Apply global translation and scaling for the whole tree
        mMatrix = mat4.translate(mMatrix, [t_x, t_y, 0]);
        mMatrix = mat4.scale(mMatrix, [treeScaleX, treeScaleY, 1.0]);
    }
...
function drawBoat(translationX, translationY = 0, scaleX = 1.0, scaleY = 1.0) {
    // initialize the model matrix to identity matrix
    mat4.identity(mMatrix);

    // applying global translation
    mMatrix = mat4.translate(mMatrix, [translationX, translationY, 0]);
...
function drawFan(rotationAngle, move = false, t_x = 0) {
    ...
    color = [0, 0, 0, 1.0];
    if (t_x != 0) {
        mMatrix = mat4.translate(mMatrix, [0.7, -0.14, 0]);
        // local scale operation for the square
    ...
    if (t_x != 0) {
        mMatrix = mat4.scale(mMatrix, [0.18, 0.18, 1.0]);
    }
    else {
...
function drawBush(move = false, t_x = 0, t_y = 0, s = 0) {
    ...
    pushMatrix(matrixStack, mMatrix);
    color = [0, 0.7, 0, 0.9];
    mMatrix = mat4.translate(mMatrix, [-0.99, -0.535, 0]);
    ...
    pushMatrix(matrixStack, mMatrix);
    color = [0, 0.4, 0, 0.9];
    mMatrix = mat4.translate(mMatrix, [-0.73, -0.53, 0]);
    mMatrix = mat4.scale(mMatrix, [0.07, 0.05, 1.0]);
    drawCircle(color, mMatrix);
    mMatrix = popMatrix(matrixStack);
...
function drawStar(x, y, scaleX, scaleY, visible = true) {
    if (!visible) return;

    color = [1, 1, 1, 1];

    // Use fixed triangle size
    const baseWidth = 0.025;   // base side (shorter)
    const height = 0.07;       // outward vertex (longer sides)

    // Apply global scaling to the whole star
    // 1st triangle (points up)
    mat4.identity(mMatrix);
    pushMatrix(matrixStack, mMatrix);
    mMatrix = mat4.translate(mMatrix, [x, y, 0]);
    mMatrix = mat4.scale(mMatrix, [scaleX, scaleY, 1.0]);
    ...
    mMatrix = mat4.scale(mMatrix, [baseWidth, height, 1.0]);
    drawTriangle(color, mMatrix);
    mMatrix = popMatrix(matrixStack);

    // 2nd triangle (points right)
    mat4.identity(mMatrix);
    pushMatrix(matrixStack, mMatrix);
    mMatrix = mat4.translate(mMatrix, [x, y, 0]);
    mMatrix = mat4.scale(mMatrix, [scaleX, scaleY, 1.0]);
    mMatrix = mat4.rotate(mMatrix, Math.PI / 2, [0, 0, 1]);
    mMatrix = mat4.translate(mMatrix, [0.06, 0.03, 0]);
    mMatrix = mat4.scale(mMatrix, [baseWidth, height, 1.0]);
    drawTriangle(color, mMatrix);
    mMatrix = popMatrix(matrixStack);

    // 3rd triangle (points down)
    mat4.identity(mMatrix);
    pushMatrix(matrixStack, mMatrix);
    mMatrix = mat4.translate(mMatrix, [x, y, 0]);
    mMatrix = mat4.scale(mMatrix, [scaleX, scaleY, 1.0]);
    ...
    // 4th triangle (points left)
    mat4.identity(mMatrix);
    pushMatrix(matrixStack, mMatrix);
    mMatrix = mat4.translate(mMatrix, [x, y, 0]);
    mMatrix = mat4.scale(mMatrix, [scaleX, scaleY, 1.0]);
    mMatrix = mat4.rotate(mMatrix, 3 * Math.PI / 2, [0, 0, 1]);
    mMatrix = mat4.translate(mMatrix, [-0.06, 0.03, 0]);
    mMatrix = mat4.scale(mMatrix, [baseWidth, height, 1.0]);
    drawTriangle(color, mMatrix);
    mMatrix = popMatrix(matrixStack);
...
// Add new variables for the second boat's motion
let translationX2 = 0.0;
let direction2 = 1;
const translationSpeed2 = 0.004; // can be different for second boat
const translationRange2 = 0.85;   // same range as first boat, or adjust as needed

let starBlinkTimer = 0;
let starVisible = true;

let starShrink = false;
let starScaleMod = 1.0;

// Blinking star parameters
let starBlinkPhase = 0.0;
let starScale = 1.0;
...
function drawScene() {
    ...
    function animate() {
        ...
        rotationAngle_moon += rotationSpeed_Moon;
        ...
        // Blinking logic: shrink for 10 frames, normal for 20 frames
        starBlinkTimer++;
        if (starBlinkTimer <= 10) {
            starScaleMod = 0.5; // shrink
        } else {
            starScaleMod = 1.0; // normal size
        }
        if (starBlinkTimer > 30) {
            starBlinkTimer = 0;
        }

        drawSky();

        // Draw the animated stars with shrinking effect
        starBlinkPhase += 0.075; // speed of blinking (adjust if needed)
        starScale = 0.8 + 0.2 * Math.sin(starBlinkPhase);
        // draw the 5 stars
        drawStar(0.55, 0.95, 0.4 * starScale, 0.4 * starScale);
        drawStar(0.3, 0.78, 0.62 * starScale, 0.62 * starScale);
        drawStar(-0.23, 0.75, 0.4 * starScale, 0.4 * starScale);
        drawStar(-0.1, 0.55, 0.3 * starScale, 0.3 * starScale);
        drawStar(-0.02, 0.65, 0.4 * starScale, 0.4 * starScale);

        // applying animation to the Moon
        drawMoon(rotationAngle_moon);

        drawCloud();

        // draw the 3 mountains
        //t_x1, t_y1, s_x, s_y, t_x2 = 0, t_y2 = 0
        drawMountain(-0.6, 0.09, 1.2, 0.2, -0.5805, 0.093);
        drawMountain(-0.076, 0.09, 1.5, 0.45, -0.028, 0.096);
        drawMountain(0.82, 0.05, 1.0, 0.3, -0.545, -0.005, true);
        ...
        // draw the trees
        // move = false, t_x = 0, t_y= 0, s_x = 0, s_y = 0
        //drawTrees(true, 0.35, 0, 0.85, 0.85)
        //drawTrees(true, -0.1, 0, 0.8, 0.8);
        drawTrees(true, 0.35, 0, 0.85, 0.85)
        drawTrees(false, -0.043, 0, 0, 0);
        drawTrees(true, -0.1, 0, 0.8, 0.8);
        //drawTrees(false, -0.043, 0, 0, 0);

        //drawTrees(true, -0.2, 0, 0.8, 0.8)

        // Second boat (smaller, higher river)
        drawBoat(translationX2, 0.02, 0.47, 0.57); // adjust y and scale as needed

        // applying back and forth motion to the boat
        // First boat (normal size, lower river)
        drawBoat(translationX, 0, 1.0, 1.0);


        // applying rotatory motion to the blades of the windmill
        drawFan(-rotationAngle, true, -0.2);
        drawFan(-rotationAngle);

        // draw the bushes
        // t_x=0, t_y=0, s=0
        drawBush(false, 0, 0, 0);
        drawBush(true, 0.534, -0.1, 0.85);
        drawBush(true, 1.48, -0.15, 1.6);
        drawBush(true, 1.85, 0.05, 1);
